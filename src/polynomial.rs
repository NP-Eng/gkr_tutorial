...
pub fn specialise<F: Field>(pol: &SparsePolynomial<F, SparseTerm>, i: usize, val: F) -> SparsePolynomial<F, SparseTerm> {
...
pub fn univariate_index<F: Field>(pol: &SparsePolynomial<F, SparseTerm>) -> Result<Option<usize>, ()> {
    ...
    for (_, term) in &pol.terms {
        for (id, pow) in zip(term.vars(), term.powers()) {
            ...
            if pow != 0 {
                match found {
                    None => found = Some(id),
                    Some(i) => if id != i {return Err(())},
                }
            }
        }
    }

    if found.is_none() {Ok(None)} else {Ok(found)}
...
pub fn partial_degree<F: Field>(pol: &SparsePolynomial<F, SparseTerm>, i: usize) -> usize {
    ...
    pol.terms.iter().map(
        |(_, term)| zip(term.vars(), term.powers()).map(
            |(id, exp)| if id == i {exp} else {0}
        ).sum() // the constructor of SparsePolynomial already groups tuples corresponding to the same variable, but we add just to be safe
    ).max().unwrap()
